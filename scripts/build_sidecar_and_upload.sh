...
sha256=$(sha256sum "src/config-server-sidecar/config-server-v${version}" | awk '{print $1}')
rm "src/config-server-sidecar/config-server-v${version}"

downloadurl="https://s3.amazonaws.com/${BUCKET}/config-server-sidecar/config-server-v${version}"
